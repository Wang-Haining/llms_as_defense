...
def grid_search_logreg(
    ...
    """
    pipeline = create_logreg_pipeline()
...
def optimize_logreg_for_corpus(
        corpus_name: str,
        data_loader,
        output_dir: Path,
        n_jobs: int = -1
    ...
    # perform grid search
    best_params, cv_score = grid_search_logreg(X_train, train_labels, n_jobs)
    ...
    with open(output_dir / f"{corpus_name.lower()}_logreg_optimization.json", "w") as f:
...
def main():
    ...
    for corpus_name, data_loader in tqdm(corpora.items(), desc="optimizing corpora"):
        try:
            results = optimize_logreg_for_corpus(
