                    ...
                    for seed_file in eval_dir.glob("seed_*.json"):
                        ...
                        if isinstance(llm_data, list) and llm_data:
                            for doc_idx, document in enumerate(llm_data):
                                ...
                                acc1 = binary_acc1_list[doc_idx] if binary_acc1_list and doc_idx < len(binary_acc1_list) else None
                                acc5 = binary_acc5_list[doc_idx] if binary_acc5_list and doc_idx < len(binary_acc5_list) else None
                                true_conf = true_conf_list[doc_idx] if true_conf_list and doc_idx < len(true_conf_list) else None
                                ...
                                if quality.get("bertscore", {}).get("bertscore_f1"):
                                    bertscore_values = quality["bertscore"]["bertscore_f1"]
                                    if isinstance(bertscore_values, list) and doc_idx < len(bertscore_values):
                                ...
                                row = {
                                    ...
                                    "accuracy@1": float(attr.get("post", {}).get("accuracy@1", 0.0)) if acc1 is None else acc1,
                                    "accuracy@5": float(attr.get("post", {}).get("accuracy@5", 0.0)) if acc5 is None else acc5,
                                    "true_class_confidence": attr.get("post", {}).get("true_class_confidence") if true_conf is None else true_conf,
                                    "entropy": attr.get("post", {}).get("entropy") if entropy is None else entropy,
                                    "bertscore": bertscore,
                                    "pinc": pinc,
