            ...
            for threat_model in THREAT_MODELS:
                for llm in LLMS:
                    ...
                    for seed_file in eval_dir.glob("seed_*.json"):
                        ...
                        binary_acc1_list, binary_acc5_list, true_conf_list, entropy_list = get_sample_metrics(
                            seed_file)
                        ...
                        attr = record[threat_model].get("attribution", {})
                        ...
                        if isinstance(llm_data, list) and llm_data:
                            for doc_idx, document in enumerate(llm_data):
                                ...
                                # Use sample-level metrics if available, otherwise use aggregates
                                acc1 = binary_acc1_list[
                                    doc_idx] if binary_acc1_list and doc_idx < len(
                                    binary_acc1_list) else None
                                acc5 = binary_acc5_list[
                                    doc_idx] if binary_acc5_list and doc_idx < len(
                                    binary_acc5_list) else None
                                true_conf = true_conf_list[
                                    doc_idx] if true_conf_list and doc_idx < len(
                                    true_conf_list) else None
                                entropy = entropy_list[
                                    doc_idx] if entropy_list and doc_idx < len(
                                    entropy_list) else None

                                # Quality metrics are often at the seed level, check if we have sample-level data
                                bertscore = None
                                pinc = None

                                # Check if we have per-sample quality metrics
                                if quality.get("bertscore", {}).get("bertscore_f1"):
                                    bertscore_values = quality["bertscore"][
                                        "bertscore_f1"]
                                    if isinstance(bertscore_values,
                                                  list) and doc_idx < len(
                                            bertscore_values):
                                        bertscore = bertscore_values[doc_idx]
                                    else:
                                ...
                                # Calculate average PINC if we have per-ngram scores
                                pinc_values = []
                                for k in range(1, 5):
                                    pinc_key = f"pinc_{k}"
                                    if pinc_key in quality.get("pinc", {}):
                                        values = quality["pinc"][pinc_key]
                                        if isinstance(values, list) and doc_idx < len(
                                                values):
                                            pinc_values.append(values[doc_idx])
                                ...
                                row = {
                                    ...
                                    "accuracy@1": float(
                                        attr.get("post", {}).get("accuracy@1",
                                                                 0.0)) if acc1 is None else acc1,
                                    "accuracy@5": float(
                                        attr.get("post", {}).get("accuracy@5",
                                                                 0.0)) if acc5 is None else acc5,
                                    "true_class_confidence": attr.get("post", {}).get(
                                        "true_class_confidence") if true_conf is None else true_conf,
                                    "entropy": attr.get("post", {}).get(
