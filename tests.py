"""
This module provides functions to analyze text-based corpora for label mappings, feature
shifts, and stylistic modifications using machine learning techniques. It includes:
- Debugging label distributions between training and test datasets.
- Analyzing feature shifts due to modifications like obfuscation and imitation.
- Summarizing key metrics and trends across multiple corpora and tasks.
...
if __name__ == "__main__":
    ...
    for corpus in ['rj', 'ebg', 'lcmc']:
        ...
        for task in tasks:
            print(f"\n{'-' * 30}\nTask: {task}")
            train_text, train_labels_raw, test_text, test_labels_raw = load_corpus(corpus, task)
            debug_info = debug_label_mapping(train_text, train_labels_raw, test_text, test_labels_raw, f"{corpus}-{task}")

    print("\nStarting feature shift analysis...")
    results = {}
    for corpus in ['rj', 'ebg']:
        ...
        results[f"{corpus}_obfuscation"] = analyze_feature_shifts(corpus, 'no_protection', 'obfuscation')

        # compare no_protection vs imitation
        results[f"{corpus}_imitation"] = analyze_feature_shifts(corpus, 'no_protection', 'imitation')
